...
Per-label branch interpreted by the wrap:
  BLOCK  -> emit OTel verdict, raise ModelInputBlockedBySplunkGate (handler NOT called)
...
import structlog
from splunkgate_core.errors import ModelInputBlockedBySplunkGate, ModelOutputBlockedBySplunkGate
...
from splunkgate_mw._first_pass import (
    cheap_first_pass,
    extract_user_text,
    truncate_input,
)
...
class SafetyModelMiddleware(AgentMiddleware):  # type: ignore[misc]
    ...
    async def model_middleware(
        ...
        if pre_verdict.verdict is VerdictLabel.BLOCK:
            ...
            raise ModelInputBlockedBySplunkGate(pre_verdict)
    ...
    async def _apply_post_scan(
        ...
        *,
        ...
        Threads `splunkgate_pre_trace_id` and `splunkgate_post_trace_id` into the
        ...
        if post_verdict.verdict is VerdictLabel.BLOCK:
            ...
            raise ModelOutputBlockedBySplunkGate(post_verdict)
...
def _build_audit_extras(
    ...
    Keys are namespaced `splunkgate_*` so they don't collide with splunklib.ai's
    ...
    if pre_verdict is not None and pre_verdict.verdict is not VerdictLabel.ALLOW:
        extras["splunkgate_pre_trace_id"] = str(pre_verdict.trace_id)
    if post_verdict.verdict is not VerdictLabel.ALLOW:
        extras["splunkgate_post_trace_id"] = str(post_verdict.trace_id)
...
def _with_extras(message: AIMessage, extras: dict[str, object]) -> AIMessage:
    ...
    The splunkgate_* keys take precedence on collision (defensive: agent code
    upstream may have set its own splunkgate_* extras — we want our values
