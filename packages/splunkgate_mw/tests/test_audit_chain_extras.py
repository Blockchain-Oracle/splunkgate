...
`splunkgate_pre_trace_id` and `splunkgate_post_trace_id` into the returned
...
from splunkgate_judges.ai_defense_types import RuleHit as InspectRuleHit
from splunkgate_mw import model_middleware as mw_mod
from splunkgate_mw.model_middleware import (
...
def test_build_audit_extras_post_modify_only() -> None:
    ...
    assert out == {"splunkgate_post_trace_id": str(post.trace_id)}
...
def test_build_audit_extras_pre_modify_post_allow() -> None:
    ...
    post = _verdict(VerdictLabel.ALLOW)
    out = _build_audit_extras(pre, post)
    assert out == {"splunkgate_pre_trace_id": str(pre.trace_id)}
...
def test_build_audit_extras_pre_and_post_both_modify() -> None:
    ...
    out = _build_audit_extras(pre, post)
    assert out == {
        "splunkgate_pre_trace_id": str(pre.trace_id),
        "splunkgate_post_trace_id": str(post.trace_id),
    }


def test_with_extras_preserves_upstream_message_extras() -> None:
    """Upstream agent extras coexist with splunkgate_* keys; splunkgate wins on collision."""
    msg = AIMessage(
        content="hello",
        calls=[],
        extras={"foo": "bar", "splunkgate_pre_trace_id": "STALE"},
    )
    new = _with_extras(msg, {"splunkgate_pre_trace_id": "NEW", "splunkgate_post_trace_id": "POST"})
    assert new.content == "hello"
    assert new.extras == {
        "foo": "bar",
        "splunkgate_pre_trace_id": "NEW",
        "splunkgate_post_trace_id": "POST",
    }
...
def test_with_extras_handles_none_existing_extras() -> None:
    ...
    new = _with_extras(msg, {"splunkgate_post_trace_id": "abc"})
    assert new.extras == {"splunkgate_post_trace_id": "abc"}


def test_splunkgate_pre_trace_id_value_is_uuid_string() -> None:
    """Trace id values are stringified UUIDs (auditor-friendly form)."""
    pre = _verdict(VerdictLabel.MODIFY)
    out = _build_audit_extras(pre, _verdict(VerdictLabel.ALLOW))
    assert out is not None
    UUID(str(out["splunkgate_pre_trace_id"]))  # raises on malformed uuid
...
async def test_pre_modify_post_allow_threads_pre_trace_id_to_response(
    ...
    assert result.message.extras == {"splunkgate_pre_trace_id": str(pre_v.trace_id)}
...
async def test_pre_modify_post_modify_threads_both_trace_ids(
    ...
    assert extras["splunkgate_pre_trace_id"] == str(pre_v.trace_id)
    assert "splunkgate_post_trace_id" in extras
    UUID(str(extras["splunkgate_post_trace_id"]))
...
async def test_pre_allow_post_modify_threads_post_trace_id_only() -> None:
    ...
    assert result.message.content == "[REDACTED]"
    extras = result.message.extras or {}
    assert "splunkgate_pre_trace_id" not in extras
    assert "splunkgate_post_trace_id" in extras
