...
import pytest
import splunkgate_mw.model_middleware as mw_mod
...
# to whatever provider is already configured (splunkgate_core conftest sets one
# when those tests run first; for standalone splunkgate_mw runs we set it here).
...
@pytest.mark.asyncio
async def test_block_raises_model_input_blocked_by_splunkgate() -> None:
    """BLOCK verdict from pre-scan → ModelInputBlockedBySplunkGate raised."""
    cfg = Config(escalate_on_first_pass_hit=False)
    mw = SafetyModelMiddleware(config=cfg)
    seen, handler = _record_handler()
    with pytest.raises(ModelInputBlockedBySplunkGate) as exc_info:
    ...
    blocked: ModelInputBlockedBySplunkGate = exc_info.value
