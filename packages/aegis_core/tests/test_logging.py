"""Behavioral tests for aegis_core.logging configuration + trace_id injection."""
...
def test_aegis_log_format_env_var_overrides_to_json(
...
def test_aegis_log_format_env_var_overrides_to_console(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("AEGIS_LOG_FORMAT", "console")
    configure_logging()
    cap = _install_capture()
    log = get_logger("test")
...
def test_explicit_dev_mode_kwarg_beats_env_var(
    ...
    """dev_mode=True must win over AEGIS_LOG_FORMAT=json env var."""
    monkeypatch.setenv("AEGIS_LOG_FORMAT", "json")
...
def test_configure_logging_does_not_raise_with_unset_env(
    ...
    monkeypatch.delenv("AEGIS_LOG_FORMAT", raising=False)
    configure_logging()
    assert os.environ.get("AEGIS_LOG_FORMAT") is None
