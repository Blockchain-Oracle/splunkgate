...
from splunkgate_core.otel import (
    EVALUATION_NAME,
    emit_verdict_event,
    severity_to_score,
)
...
def test_evaluation_name_attr_is_splunkgate_safety_verdict(
    exporter: InMemorySpanExporter,
) -> None:
    span = _emit_inside_span(exporter, _verdict())
...
def test_splunkgate_surface_attr_matches_verdict_surface(
    exporter: InMemorySpanExporter,
) -> None:
    span = _emit_inside_span(exporter, _verdict())
    attrs = dict(span.events[0].attributes or {})
    assert attrs["splunkgate.surface"] == "mw_model"


def test_splunkgate_rules_attr_is_list_of_rule_names(
    exporter: InMemorySpanExporter,
) -> None:
    rules = [
    ...
    attrs = dict(span.events[0].attributes or {})
    rules_attr = attrs["splunkgate.rules"]
    assert list(rules_attr) == ["Prompt Injection", "PII"]  # type: ignore[arg-type]


def test_splunkgate_trace_id_is_stringified_uuid(exporter: InMemorySpanExporter) -> None:
    v = _verdict()
    span = _emit_inside_span(exporter, v)
    attrs = dict(span.events[0].attributes or {})
    assert attrs["splunkgate.trace_id"] == str(v.trace_id)


def test_splunkgate_agent_id_attr_absent_when_none(
    exporter: InMemorySpanExporter,
) -> None:
    ...
    attrs = dict(span.events[0].attributes or {})
    assert "splunkgate.agent_id" not in attrs


def test_splunkgate_agent_id_attr_present_when_supplied(
    exporter: InMemorySpanExporter,
) -> None:
    ...
    attrs = dict(span.events[0].attributes or {})
    assert attrs["splunkgate.agent_id"] == "support-agent-7"
