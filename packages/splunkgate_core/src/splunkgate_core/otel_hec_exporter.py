...
Per ADR-005 the default sourcetype is `cisco_ai_defense:splunkgate_verdict`,
which colocates SplunkGate events with Cisco Security Cloud (Splunkbase app
...
`SPLUNKGATE_DEV_INSECURE_TLS=1` opt-in for self-signed certs, logs a WARN.
"""
...
DEFAULT_SOURCETYPE: Final[str] = "cisco_ai_defense:splunkgate_verdict"
DEFAULT_SOURCE: Final[str] = "splunkgate-otel"
...
class SplunkHECExporter(SpanExporter):
    ...
    SplunkGate-specific by design — not a generic OTel→Splunk shim. If a future
...
def configure_hec_exporter(  # noqa: PLR0913 — kwarg surface mirrors Splunk HEC config
    ...
    *,
    ...
    Env-var fallbacks: SPLUNKGATE_SPLUNK_HEC_URL, SPLUNKGATE_SPLUNK_HEC_TOKEN,
    SPLUNKGATE_SPLUNK_INDEX, SPLUNKGATE_SPLUNK_HEC_SOURCETYPE. Kwargs win.

    TLS: defaults to verify=True. SPLUNKGATE_DEV_INSECURE_TLS=1 opts in to
    ...
    url = hec_url or _resolve_env("SPLUNKGATE_SPLUNK_HEC_URL")
    token = hec_token or _resolve_env("SPLUNKGATE_SPLUNK_HEC_TOKEN")
    idx = index or _resolve_env("SPLUNKGATE_SPLUNK_INDEX") or DEFAULT_INDEX
    stype = sourcetype or _resolve_env("SPLUNKGATE_SPLUNK_HEC_SOURCETYPE") or DEFAULT_SOURCETYPE

    if not url:
        msg = "SPLUNKGATE_SPLUNK_HEC_URL not set (or pass hec_url kwarg)"
        raise ConfigError(msg)
    if not token:
        msg = "SPLUNKGATE_SPLUNK_HEC_TOKEN not set (or pass hec_token kwarg)"
        raise ConfigError(msg)

    insecure = _resolve_env("SPLUNKGATE_DEV_INSECURE_TLS") == "1"
    if insecure:
        _logger.warning(
            "hec_tls_verify_disabled",
            reason="SPLUNKGATE_DEV_INSECURE_TLS=1 is set; production deployments must verify TLS",
        )
