"""SplunkGate Verdict — the single domain type every surface emits.

The shape locked here flows through:
- S1 middleware (splunkgate_mw): every tool/subagent/model boundary
- S2 MCP server (splunkgate_mcp): tool outputSchema is Verdict.model_json_schema()
- S3 DefenseClaw integration: SplunkGate HTTP sink wraps responses as Verdict
- S4 Splunk app: events land under sourcetype cisco_ai_defense:splunkgate_verdict
...
class Verdict(BaseModel):
    """The verdict returned by every SplunkGate surface.
