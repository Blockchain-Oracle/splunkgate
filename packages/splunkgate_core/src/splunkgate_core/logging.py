"""SplunkGate structlog configuration + stable key conventions.

Every surface calls `from splunkgate_core.logging import get_logger` and gets a
pre-configured BoundLogger that:

1. Renders JSON in prod (HEC-ingestible by Splunk) and console-pretty in dev
   (configurable via SPLUNKGATE_LOG_FORMAT={json|console} env var or `dev_mode`
   kwarg to `configure_logging()`)
2. Emits the stable SplunkGate key set on every record:
   - `event`     human-readable event name
   - `verdict`   "ALLOW" | "BLOCK" | "MODIFY" | "REVIEW"
   - `severity`  "NONE_SEVERITY" | "LOW" | "MEDIUM" | "HIGH"
   - `trace_id`  str(UUID) — auto-injected from splunkgate_core.trace.current_trace_id()
...
"""The stable set of structured-log keys SplunkGate emits.
...
def _resolve_dev_mode(*, dev_mode_kwarg: bool | None) -> bool:
    ...
    env = os.environ.get("SPLUNKGATE_LOG_FORMAT", "").strip().lower()
    if env == "json":
        return False
    if env == "console":
        return True
...
def configure_logging(*, dev_mode: bool | None = None) -> None:
    ...
    Resolution order for dev/prod selection:
      ...
      2. SPLUNKGATE_LOG_FORMAT env var ("json" → prod, "console" → dev)
