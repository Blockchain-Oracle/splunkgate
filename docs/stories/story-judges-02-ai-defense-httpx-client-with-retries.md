...
**So that** every Aegis surface can submit prompts/responses for inspection without each surface re-implementing auth, region selection, retry, or log shape
...
Given the test file packages/aegis_judges/tests/test_ai_defense_client.py exists
When  `uv run pytest packages/aegis_judges/tests/test_ai_defense_client.py -v` runs
Then  ≥ 10 tests pass and 0 fail

Given the src files for this story
When  `uv run mypy --strict packages/aegis_judges/src/aegis_judges/ai_defense.py packages/aegis_judges/src/aegis_judges/_regions.py packages/aegis_judges/src/aegis_judges/_errors.py` runs
Then  exit code is 0
...
When  `grep -rE "(mock|fake|dummy|hardcoded|simulated)" packages/aegis_judges/src/aegis_judges/ai_defense.py` runs
...
```bash
# Tests pass
uv run pytest packages/aegis_judges/tests/test_ai_defense_client.py -v 2>&1 | grep -cE "PASSED"
# Must output >= 10

# Region routing sanity
uv run python -c "
from aegis_judges._regions import REGION_BASE_URLS, INSPECT_CHAT_PATH
...
# Strict typecheck
uv run mypy --strict packages/aegis_judges/src/aegis_judges/ai_defense.py packages/aegis_judges/src/aegis_judges/_regions.py packages/aegis_judges/src/aegis_judges/_errors.py
# Must exit 0

# 400-LOC cap on each new file
for f in packages/aegis_judges/src/aegis_judges/ai_defense.py packages/aegis_judges/src/aegis_judges/_regions.py packages/aegis_judges/src/aegis_judges/_errors.py; do
  lines=$(wc -l < "$f")
...
grep -rE "(mock|fake|dummy|hardcoded|simulated)" packages/aegis_judges/src/aegis_judges/ai_defense.py
# Must output nothing

# No verify=False slipped in
grep -nE "verify\s*=\s*False" packages/aegis_judges/src/aegis_judges/ai_defense.py
...
- Structlog keys: `event`, `trace_id`, `region`, `rules_count`, `latency_ms`, `severity`, `is_safe`. Stable names matter for downstream Splunk parsing (`cisco_ai_defense:aegis_verdict` sourcetype).
