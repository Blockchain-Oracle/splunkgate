...
**As a** Splunk Build-a-thon-style judge who wants to see SplunkGate's Splunk app installed locally to demo it
**I want to** run a single `scripts/build_splunk_app_tgz.sh` invocation that produces a Splunkbase-ready `.tgz` artifact in `dist/splunkgate_app-<version>.tgz` matching the conventions in `../../../context/05-splunk-core/08-app-packaging-and-conf-files.md`, and read `docs/splunkbase-submission-checklist.md` to confirm every Splunkbase requirement is met
...
- `scripts/build_splunk_app_tgz.sh` — NEW — bash script: validates `splunk_apps/splunkgate_app/default/app.conf` has a `version = X.Y.Z` line (semver), reads that version, strips dev cruft (`__pycache__/`, `.pytest_cache/`, `*.pyc`, `.DS_Store`, `tests/`, `.appinspect.*.yaml.bak`, `appinspect-report.json`, etc.), runs AppInspect once (via story-app-11's `run_appinspect.sh`) to confirm zero-error pre-flight, then creates `dist/splunkgate_app-<version>.tgz` via `tar -czf dist/splunkgate_app-<version>.tgz -C splunk_apps splunkgate_app/` from the repo root. Computes and prints sha256 + size of the resulting artifact. Exit 1 if app.conf version line is missing or non-semver, or if AppInspect fails.
- `scripts/_strip_dev_cruft.sh` — NEW — bash helper invoked by `build_splunk_app_tgz.sh` that walks `splunk_apps/splunkgate_app/` and removes dev-only files (listed in a stable inline allowlist of patterns) before packaging. Idempotent. Logs which files were removed.
- `splunk_apps/splunkgate_app/META-INF/manifest.json` — NEW — Splunk-standard app manifest per `../../../context/05-splunk-core/08-app-packaging-and-conf-files.md`. Fields per the documented schema (see Notes): `schemaVersion`, `info.id`, `info.title`, `info.version`, `info.author`, `info.license`, `info.releaseDate`, `info.description`, `info.classification.intendedAudience`, `info.classification.categories`, `info.classification.developmentStatus`, `dependencies`, `targetWorkloads` (`["_search_head_instances"]`). Match the `app.conf` version field exactly.
...
- `scripts/verify_splunkbase_artifact.sh` — NEW — runs the artifact end-to-end checks: extracts the tarball to a temp dir, runs `splunk-appinspect inspect <extracted>/splunkgate_app --included-tags cloud`, validates manifest.json schema (Pydantic model `eval/.../manifest_schema.py` or inline Python), checks for the required README + LICENSE files in the extracted tree. Exit 0 if all pass. Used by `release.yml` (story-cicd-08).
- `tests/test_build_artifact.py` — NEW (at repo root tests dir, per existing CI pattern) — ≥ 10 tests: `build_splunk_app_tgz.sh` produces `dist/splunkgate_app-<version>.tgz`; the produced archive contains `splunkgate_app/default/app.conf`; contains `splunkgate_app/README`; contains `splunkgate_app/META-INF/manifest.json`; does NOT contain `__pycache__/` or `*.pyc`; does NOT contain `tests/`; the version in `manifest.json` equals the version in `app.conf`; the version string matches semver regex; the script runs idempotently (second invocation produces an artifact with the same sha256); `verify_splunkbase_artifact.sh` exits 0 on a freshly built artifact.
- `splunk_apps/splunkgate_app/LICENSE` — NEW (or symlink) — Apache-2.0 license file at the app root (required by Splunkbase per the submission checklist).
...
```
Given splunk_apps/splunkgate_app/META-INF/manifest.json exists
When  `python -c "import json; d=json.load(open('splunk_apps/splunkgate_app/META-INF/manifest.json')); print(d['info']['id'], d['info']['version'])"` runs
Then  exit code is 0
And   output starts with "splunkgate_app "

Given splunk_apps/splunkgate_app/META-INF/manifest.json and splunk_apps/splunkgate_app/default/app.conf
...
When  it completes
Then  exit code is 0
And   a single tarball matching `dist/splunkgate_app-*.tgz` exists
And   the printed sha256 is a 64-character hex string

Given the produced tarball
When  `tar -tzf dist/splunkgate_app-*.tgz | head -20` runs
Then  the listing includes `splunkgate_app/default/app.conf`
And   includes `splunkgate_app/README`
And   includes `splunkgate_app/META-INF/manifest.json`
And   does not include `__pycache__`, `*.pyc`, `.DS_Store`, or `tests/`

Given the produced tarball
When  `bash scripts/verify_splunkbase_artifact.sh dist/splunkgate_app-*.tgz` runs
Then  exit code is 0
...
Given the LICENSE file at splunk_apps/splunkgate_app/LICENSE
...
test -f splunk_apps/splunkgate_app/META-INF/manifest.json
uv run python - <<'PY'
import json, re
m = json.load(open("splunk_apps/splunkgate_app/META-INF/manifest.json"))
assert m["info"]["id"] == "splunkgate_app", m["info"]
mv = m["info"]["version"]
av = None
for line in open("splunk_apps/splunkgate_app/default/app.conf"):
...
test -f splunk_apps/splunkgate_app/LICENSE
head -5 splunk_apps/splunkgate_app/LICENSE | grep -q 'Apache License'
head -5 splunk_apps/splunkgate_app/LICENSE | grep -q 'Version 2.0'
...
sha_a=$(find dist -name 'splunkgate_app-*.tgz' -exec sha256sum {} \; | awk '{print $1}')
n=$(find dist -name 'splunkgate_app-*.tgz' | wc -l)
[ "$n" -eq 1 ] || { echo "FAIL: expected 1 tarball, got $n"; exit 1; }

# 4. Tarball contents are clean
artifact=$(find dist -name 'splunkgate_app-*.tgz' | head -1)
tar -tzf "$artifact" | grep -q 'splunkgate_app/default/app.conf'
tar -tzf "$artifact" | grep -q 'splunkgate_app/README'
tar -tzf "$artifact" | grep -q 'splunkgate_app/META-INF/manifest.json'
...
rm -rf dist
bash scripts/build_splunk_app_tgz.sh
sha_b=$(find dist -name 'splunkgate_app-*.tgz' -exec sha256sum {} \; | awk '{print $1}')
...
for f in \
  ...
  splunk_apps/splunkgate_app/META-INF/manifest.json \
...
- **Per `docs/architecture.md` § "Submission checklist gates"**, the SplunkGate-specific gates are: (a) `splunk-appinspect inspect splunk_apps/splunkgate_app/` passes with zero error-severity findings (this gate is implemented by story-app-11 + this story's `verify_splunkbase_artifact.sh`); (b) `LICENSE` file is Apache-2.0 at the app root; (c) `README` extension-less file at app root. All three must be present in the produced tarball.
...
- **Per `../../../context/11-prior-art/01-build-a-thon-2025-deep-read.md`**, CIMplicity AI ships as a Splunkbase release; DNS Guard AI same. Both are TGZ distributions. The TGZ root contains the `splunkgate_app/` directory (NOT bare conf files at the top level). This is what `tar -czf ... -C splunk_apps splunkgate_app/` produces — the `-C splunk_apps` strips the `splunk_apps/` parent prefix.
...
- **Per `docs/architecture.md` § "Repo structure" (line 117)**: `splunk_apps/splunkgate_app/` is the package root. The tarball must contain `splunkgate_app/` as the top-level directory inside the archive — Splunk expects this convention.
