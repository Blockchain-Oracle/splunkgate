...
**As a** Splunk admin browsing the Splunk Web Apps menu after installing splunkgate_app
**I want to** see a distinct SplunkGate app icon (and a 2x hi-DPI variant) in the launcher and an alternate icon on the App: Manager page, instead of the generic Splunk app placeholder
...
```
Given splunk_apps/splunkgate_app/static/appIcon.png exists
When  python -c "from PIL import Image; im = Image.open('splunk_apps/splunkgate_app/static/appIcon.png'); print(im.size)" runs
Then  output is "(36, 36)"

Given splunk_apps/splunkgate_app/static/appIcon_2x.png exists
When  python -c "from PIL import Image; im = Image.open(...); print(im.size)" runs
Then  output is "(72, 72)"

Given splunk_apps/splunkgate_app/static/appIconAlt.png exists
When  python -c "from PIL import Image; im = Image.open(...); print(im.size)" runs
Then  output is "(36, 36)"

Given splunk_apps/splunkgate_app/static/appIconAlt_2x.png exists
When  python -c "from PIL import Image; im = Image.open(...); print(im.size)" runs
Then  output is "(72, 72)"

Given splunk_apps/splunkgate_app/static/screenshot.png exists
When  python -c "from PIL import Image; im = Image.open(...); print(im.size)" runs
...
test -f splunk_apps/splunkgate_app/static/appIcon.png
test -f splunk_apps/splunkgate_app/static/appIcon_2x.png
test -f splunk_apps/splunkgate_app/static/appIconAlt.png
test -f splunk_apps/splunkgate_app/static/appIconAlt_2x.png
test -f splunk_apps/splunkgate_app/static/screenshot.png

# 2. PIL verifies dimensions
uv run python - <<'PY'
from PIL import Image
expected = {
    "splunk_apps/splunkgate_app/static/appIcon.png": (36, 36),
    "splunk_apps/splunkgate_app/static/appIcon_2x.png": (72, 72),
    "splunk_apps/splunkgate_app/static/appIconAlt.png": (36, 36),
    "splunk_apps/splunkgate_app/static/appIconAlt_2x.png": (72, 72),
}
...
# Screenshot >= 1280x720
ss = Image.open("splunk_apps/splunkgate_app/static/screenshot.png")
...
for f in splunk_apps/splunkgate_app/static/appIcon.png splunk_apps/splunkgate_app/static/appIcon_2x.png splunk_apps/splunkgate_app/static/appIconAlt.png splunk_apps/splunkgate_app/static/appIconAlt_2x.png splunk_apps/splunkgate_app/static/screenshot.png; do
...
for f in appIcon.png appIcon_2x.png appIconAlt.png appIconAlt_2x.png; do
  ...
  committed=$(md5sum "splunk_apps/splunkgate_app/static/$f" | cut -d' ' -f1)
...
- The glyph itself: a simple shield outline (an SplunkGate is a shield in Greek mythology — apt) OR a stylized "A" inside a circle. Either works; pick whichever the script generates more legibly at 36x36. Don't add text — at 36x36, text is illegible.
...
- `screenshot.png` is the Splunkbase listing image; minimum 1280x720 per Splunkbase submission docs. The placeholder shows: dark background, large "SplunkGate" wordmark in Splunk Sans, "Real-time AI agent safety verdicts" tagline beneath. Story-app-12 swaps this for an actual dashboard screenshot before Splunkbase submission. Don't over-invest in the placeholder.
- `scripts/generate_app_icons.py` lives OUTSIDE `splunk_apps/splunkgate_app/` to preserve the DNS Guard "zero Python in app dir" pattern per `../../../context/11-prior-art/01-build-a-thon-2025-deep-read.md`. The script's job is to regenerate icons at build time if a designer eventually replaces the glyph spec; until then, the committed PNGs are the canonical artifacts.
- The script CLI: `python scripts/generate_app_icons.py --output-dir <path>` — outputs the 4 icon files and screenshot.png into `<path>`. Default `<path>` = `splunk_apps/splunkgate_app/static/`. Easy to call from CI for regeneration verification.
