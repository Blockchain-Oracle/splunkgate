...
**I want to** see each SplunkGate verdict tagged with the MITRE ATLAS technique ID(s) corresponding to its Cisco AI Defense rule names
**So that** an SplunkGate block on a prompt-injection attempt pivots directly into my ATLAS-aligned detection workflow without re-mapping by hand
...
- `splunk_apps/splunkgate_app/lookups/atlas_technique_mapping.csv` — NEW — CSV with columns `rule_name,atlas_technique_id,atlas_technique_name,atlas_tactic_id`. Maps each of the 11 Cisco AI Defense rule names to MITRE ATLAS technique IDs. See "Acceptance criteria" for the exact required rows.
- `splunk_apps/splunkgate_app/default/transforms.conf` — UPDATE — add a `[atlas_technique_mapping]` stanza pointing at the CSV.
- `splunk_apps/splunkgate_app/default/savedsearches.conf` — UPDATE — modify the `splunkgate_verdict_ingest` saved search (defined in story-app-03) to enrich verdicts with `atlas_technique_id` + `atlas_tactic_id` via `lookup atlas_technique_mapping rule_name OUTPUT atlas_technique_id, atlas_technique_name, atlas_tactic_id`.
- `docs/architecture.md` — UPDATE — add a one-line entry to ADR-005's "SplunkGate events emit to `cisco_ai_defense:splunkgate_verdict` sourcetype" noting that the sourcetype also carries ATLAS technique IDs as enriched fields.
...
```
Given splunk_apps/splunkgate_app/lookups/atlas_technique_mapping.csv exists
When  `awk -F',' 'NR>1 {print $1}' splunk_apps/splunkgate_app/lookups/atlas_technique_mapping.csv | sort -u | wc -l` runs
...
Given splunk_apps/splunkgate_app/default/transforms.conf
When  the file is parsed for the [atlas_technique_mapping] stanza
Then  it references `filename = atlas_technique_mapping.csv`

Given splunk_apps/splunkgate_app/default/savedsearches.conf
When  the `splunkgate_verdict_ingest` saved search definition is read
...
test "$(awk -F',' 'NR>1 {print $1}' splunk_apps/splunkgate_app/lookups/atlas_technique_mapping.csv | sort -u | wc -l)" -eq 11 || exit 1

# Every ATLAS technique ID is well-formed
awk -F',' 'NR>1 {print $2}' splunk_apps/splunkgate_app/lookups/atlas_technique_mapping.csv | grep -vE '^AML\.T[0-9]{4}(\.[0-9]{3})?$' && exit 1
...
for rule in "Code Detection" "Harassment" "Hate Speech" "PCI" "PHI" "PII" "Prompt Injection" "Profanity" "Sexual Content & Exploitation" "Social Division & Polarization" "Violence & Public Safety Threats"; do
  grep -qF "$rule," splunk_apps/splunkgate_app/lookups/atlas_technique_mapping.csv || { echo "Missing rule: $rule"; exit 1; }
done
...
- Do NOT modify any other story's saved search. Story-app-03 owns `splunkgate_verdict_ingest`; this story extends that single search with a `lookup` clause only.
