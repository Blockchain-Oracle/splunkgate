...
**As a** SplunkGate developer running the full test suite without a Splunk Cloud tenant
**I want to** flip a mock toggle and have `FoundationSecExplainer.explain()` return a deterministic security-domain explanation string, and have a separate live integration test that runs only when `SPLUNKGATE_SPLUNK_HEC_TOKEN` (or equivalent Splunk auth env vars) is set
...
- `packages/splunkgate_judges/src/splunkgate_judges/foundation_sec_mock.py` — NEW — `MockFoundationSecExplainer` class implementing the same `async def explain(ctx: VerdictContext) -> str` interface; deterministic explanation generation that combines the verdict context fields into a security-domain-vocabulary explanation (must mention at minimum: severity, the rule names, an interpretive sentence) — vocabulary set MUST include at least 8 of: `analyst`, `verdict`, `risk`, `policy`, `prompt`, `injection`, `exfiltration`, `safeguard`, `mitigation`, `triage`, `detection`, `classification`, `evidence`, `compliance`
- `packages/splunkgate_judges/src/splunkgate_judges/foundation_sec.py` — UPDATE — add `FoundationSecExplainer.from_env()` factory: if `SPLUNKGATE_FOUNDATION_SEC_MOCK=1`, returns `MockFoundationSecExplainer`; if `SPLUNKGATE_FOUNDATION_SEC_DISABLED=1`, returns a `NullExplainer` that always returns `""`; else constructs the live explainer using `SplunkSearchClient.from_env()` which reads `SPLUNKGATE_SPLUNK_HOST`, `SPLUNKGATE_SPLUNK_PORT` (default 8089), `SPLUNKGATE_SPLUNK_HEC_TOKEN` OR `SPLUNKGATE_SPLUNK_TOKEN` (session token — preferred for search-job REST), and raises `FoundationSecAuthError` if neither token env var is set
- `packages/splunkgate_judges/tests/test_foundation_sec_mock.py` — NEW — ≥ 10 tests: explain returns a non-empty string; explain returns identical output for identical input (deterministic); explain output mentions the severity; explain output mentions each rule name verbatim; explain output contains ≥ 4 security-domain vocabulary terms from the required set; output length is between 80 and 800 chars; concurrent invocations are independent; `from_env()` with `SPLUNKGATE_FOUNDATION_SEC_MOCK=1` returns the mock; `from_env()` with `SPLUNKGATE_FOUNDATION_SEC_DISABLED=1` returns the null explainer and `explain()` returns `""`; `from_env()` with neither flag and missing Splunk env vars raises `FoundationSecAuthError`
- `packages/splunkgate_judges/tests/integration/test_foundation_sec_live.py` — NEW — gated on `SPLUNKGATE_SPLUNK_HEC_TOKEN` AND `SPLUNKGATE_SPLUNK_HOST`; skipped when either is unset; when set, builds a `VerdictContext` with severity HIGH + PII rule + offending text containing an SSN-like pattern, dispatches via the live explainer, asserts the returned explanation: is non-empty, has length ≥ 60, contains at least 3 security-domain vocabulary terms from the same set the mock uses; runs exactly ONE search per CI invocation (quota-respectful)
...
Given SPLUNKGATE_FOUNDATION_SEC_MOCK=1 is set
When  FoundationSecExplainer.from_env() is called
Then  the returned object's class.__name__ is "MockFoundationSecExplainer"

Given SPLUNKGATE_FOUNDATION_SEC_DISABLED=1 is set
When  from_env() is called and explain(ctx) is awaited
Then  the returned string is exactly ""

Given SPLUNKGATE_FOUNDATION_SEC_MOCK is unset, SPLUNKGATE_FOUNDATION_SEC_DISABLED is unset, SPLUNKGATE_SPLUNK_HEC_TOKEN is unset, SPLUNKGATE_SPLUNK_TOKEN is unset
When  from_env() is called
Then  FoundationSecAuthError is raised mentioning at least one of "SPLUNKGATE_SPLUNK_HEC_TOKEN" or "SPLUNKGATE_SPLUNK_TOKEN"

Given SPLUNKGATE_SPLUNK_HEC_TOKEN is unset OR SPLUNKGATE_SPLUNK_HOST is unset
When  `uv run pytest packages/splunkgate_judges/tests/integration/test_foundation_sec_live.py -v` runs
Then  every test is skipped and exit code is 0

Given both SPLUNKGATE_SPLUNK_HEC_TOKEN and SPLUNKGATE_SPLUNK_HOST are set
...
Given `uv run pytest packages/splunkgate_judges/tests/test_foundation_sec_mock.py -v`
When  it runs
Then  ≥ 10 tests pass and 0 fail

Given strict mypy on the new src files
When  `uv run mypy --strict packages/splunkgate_judges/src/splunkgate_judges/foundation_sec_mock.py packages/splunkgate_judges/src/splunkgate_judges/foundation_sec.py` runs
Then  exit code is 0
...
```bash
# Mock tests pass
uv run pytest packages/splunkgate_judges/tests/test_foundation_sec_mock.py -v 2>&1 | grep -cE "PASSED"
# Must output >= 10

# Live test gates correctly (no env → all skipped, exit 0)
env -u SPLUNKGATE_SPLUNK_HEC_TOKEN -u SPLUNKGATE_SPLUNK_TOKEN uv run pytest packages/splunkgate_judges/tests/integration/test_foundation_sec_live.py -v 2>&1 | grep -E "(SKIPPED|skipped)"
# Must show at least one SKIPPED
env -u SPLUNKGATE_SPLUNK_HEC_TOKEN -u SPLUNKGATE_SPLUNK_TOKEN uv run pytest packages/splunkgate_judges/tests/integration/test_foundation_sec_live.py
# Must exit 0

# Mock determinism + vocabulary sanity
SPLUNKGATE_FOUNDATION_SEC_MOCK=1 uv run python -c "
import asyncio
from uuid import UUID
from splunkgate_judges._explanation_prompt import VerdictContext
from splunkgate_judges.foundation_sec import FoundationSecExplainer
...
SPLUNKGATE_FOUNDATION_SEC_DISABLED=1 uv run python -c "
import asyncio
from uuid import UUID
from splunkgate_judges._explanation_prompt import VerdictContext
from splunkgate_judges.foundation_sec import FoundationSecExplainer
...
# Strict typecheck
uv run mypy --strict packages/splunkgate_judges/src/splunkgate_judges/foundation_sec_mock.py packages/splunkgate_judges/src/splunkgate_judges/foundation_sec.py
# Must exit 0

# 400-LOC cap
for f in packages/splunkgate_judges/src/splunkgate_judges/foundation_sec_mock.py packages/splunkgate_judges/src/splunkgate_judges/foundation_sec.py; do
  lines=$(wc -l < "$f")
...
- Splunk auth env var preference: `SPLUNKGATE_SPLUNK_TOKEN` (session token) is preferred for issuing search jobs via REST; `SPLUNKGATE_SPLUNK_HEC_TOKEN` is the HEC-write token used by Surface 4. The live test accepts either as a presence signal but the `SplunkSearchClient.from_env()` should prefer `SPLUNKGATE_SPLUNK_TOKEN` if both are set, and document this in its docstring with a citation to `../../../context/06-splunk-ai-stack/07-foundation-sec-on-splunk.md`.
...
- Cisco AI Defense Explorer Edition (`https://explorer.aidefense.cisco.com/`, March 23 2026 launch, free US-corp-email signup) is the AI Defense demo path. For Foundation-Sec the demo path is Abu's Splunk Cloud tenant — the demo screencast can deliberately flip `SPLUNKGATE_FOUNDATION_SEC_MOCK=1` mid-flow to demonstrate that the explainer is replaceable without touching verdict logic.
