...
**As a** Splunk admin (or `splunk-appinspect`) installing the SplunkGate app on Splunk Cloud 10.4 or Enterprise 9.4+
**I want to** drop the `splunkgate_app` directory into `$SPLUNK_HOME/etc/apps/`, restart Splunkd, and see the app appear in the Apps menu with proper version + label + visibility
...
```
Given splunk_apps/splunkgate_app/default/app.conf exists
When  grep -c "^\[install\]\|^\[launcher\]\|^\[ui\]\|^\[package\]\|^\[triggers\]" runs against it
Then  the count is exactly 5

Given splunk_apps/splunkgate_app/default/app.conf is parsed
When  grep "^version = " runs
Then  the value is exactly "1.0.0"

Given splunk_apps/splunkgate_app/default/app.conf is parsed
When  grep "^id = " runs
Then  the value is exactly "splunkgate_app"

Given splunk_apps/splunkgate_app/README exists
...
Given splunk_apps/splunkgate_app/metadata/default.meta exists
When  grep -c "^export = system" runs
Then  count >= 7 (eventtypes, props, transforms, lookups, macros, collections, tags exported)

Given splunk_apps/splunkgate_app/default/data/ui/nav/default.xml exists
When  python -c "import xml.etree.ElementTree as ET; ET.parse('splunk_apps/splunkgate_app/default/data/ui/nav/default.xml')" runs
Then  exit code is 0 (well-formed XML)

Given splunk-appinspect is installed
When  splunk-appinspect inspect splunk_apps/splunkgate_app/ --mode test --included-tags cloud runs
...
test -f splunk_apps/splunkgate_app/default/app.conf
test -f splunk_apps/splunkgate_app/metadata/default.meta
test -f splunk_apps/splunkgate_app/README
test -f splunk_apps/splunkgate_app/default/data/ui/nav/default.xml

# 2. app.conf has the five required stanzas with correct values
grep -q '^version = 1.0.0$' splunk_apps/splunkgate_app/default/app.conf
grep -q '^id = splunkgate_app$' splunk_apps/splunkgate_app/default/app.conf
grep -q '^is_visible = 1$' splunk_apps/splunkgate_app/default/app.conf
grep -q '^label = SplunkGate' splunk_apps/splunkgate_app/default/app.conf
grep -q '^is_configured = 0$' splunk_apps/splunkgate_app/default/app.conf
test "$(grep -cE '^\[(install|launcher|ui|package|triggers)\]$' splunk_apps/splunkgate_app/default/app.conf)" -eq 5

# 3. README mentions Splunk compatibility line verbatim
grep -q '9.4, 10.0, 10.1, 10.2, 10.3, 10.4' splunk_apps/splunkgate_app/README
grep -q 'Apache-2.0' splunk_apps/splunkgate_app/README
...
for ns in eventtypes props transforms lookups macros collections tags; do
  grep -A1 "^\[${ns}\]$" splunk_apps/splunkgate_app/metadata/default.meta | grep -q '^export = system$'
done

# 5. nav default.xml is well-formed
python -c "import xml.etree.ElementTree as ET; ET.parse('splunk_apps/splunkgate_app/default/data/ui/nav/default.xml')"
...
for f in splunk_apps/splunkgate_app/default/app.conf splunk_apps/splunkgate_app/metadata/default.meta splunk_apps/splunkgate_app/README splunk_apps/splunkgate_app/default/data/ui/nav/default.xml; do
...
- Author field in `[launcher]` is "SplunkGate Project" — anonymized per Apache-2.0 norms; sponsor credits live in top-level README.
...
- Do NOT create `bin/` or any Python scripts in `splunk_apps/splunkgate_app/` — DNS Guard's win was the no-Python-in-app pattern. All Python lives under `packages/`.
