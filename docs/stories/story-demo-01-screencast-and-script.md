...
**As a** Devpost judge with a 3-minute time budget for the Aegis demo video (Devpost rule: judges not required to watch beyond)
...
Given docs/demo/script.md exists
When  grep -F "[aegis] verdict=BLOCK severity=HIGH rules=[Prompt Injection]" docs/demo/script.md runs
...
Then  the total duration value extracted is <= 180 (under 3 minutes per ../../../context/01-prizes-tracks.md submission requirement)
...
Given docs/demo/terminal-script.sh exists
When  grep -F "AEGIS_AI_DEFENSE_MOCK=true" docs/demo/terminal-script.sh runs
Then  exactly one match (mock-mode env var hardcoded so demo recording does not require Cisco API access)

Given docs/demo/terminal-script.sh exists
When  grep -F "AEGIS_PROFILE=fsi" docs/demo/terminal-script.sh runs
...
Given docs/demo/README.md exists
When  grep -F "AEGIS_DEMO_PENDING" docs/demo/README.md runs
...
When  AEGIS_AI_DEFENSE_MOCK=true AEGIS_PROFILE=fsi bash docs/demo/terminal-script.sh runs
...
grep -qF '[aegis] verdict=BLOCK severity=HIGH rules=[Prompt Injection]' docs/demo/script.md
...
grep -qF "AEGIS_AI_DEFENSE_MOCK=true" docs/demo/terminal-script.sh
grep -qF "AEGIS_PROFILE=fsi" docs/demo/terminal-script.sh
...
grep -qF "AEGIS_DEMO_PENDING" docs/demo/README.md
...
AEGIS_AI_DEFENSE_MOCK=true AEGIS_PROFILE=fsi bash docs/demo/terminal-script.sh | tee /tmp/aegis-demo-stdout.txt
grep -qF "verdict=BLOCK" /tmp/aegis-demo-stdout.txt
...
git diff main...HEAD -- 'docs/demo/' | grep -E "^\+" | grep -iE "(mock|fake|dummy|hardcoded|simulated)" | grep -v "test\|spec\|asciinema\|demo\|AEGIS_AI_DEFENSE_MOCK\|README\|comment" || true
```
...
- Per `../../../context/01-prizes-tracks.md`, demo video must be < 3 minutes and hosted on YouTube/Vimeo/Youku. Pick YouTube (largest audience + simplest unlisted-then-public toggle). The script targets 90s ± 10s — leaves a full 60-90s buffer under the cap for re-takes + intro/outro padding without going over.
- Per `../../../context/01-prizes-tracks.md`, no unlicensed third-party trademarks, music, or copyrighted material — no background music in the recording, no Splunk/Cisco logos in any overlay, no licensed font without an SIL/OFL license (use the same DejaVu Sans / Inter the banner generator uses).
- Per `../../../context/01-prizes-tracks.md`, non-English audio needs English translation — we record in English; no captions story needed for v1, but YouTube auto-generates captions on upload (manually review for accuracy before publishing).
...
- Per `docs/PRD.md` § "Demo moment beat 3", the BLOCK verdict console line is verbatim `[aegis] verdict=BLOCK severity=HIGH rules=[Prompt Injection] explanation="..."`. BDD #3 enforces the literal prefix. Pin this format string in `packages/aegis_mw/src/aegis_mw/model_middleware.py` (from story-mw-03) — if that story changes the log line shape, this story's BDD fails and either this story or the upstream story must be re-aligned.
- Per `../../../context/sources/docs-saved/splunk-cloud-live-verification-2026-06-02.md`, Abu's Splunk Cloud demo instance is verified at 10.4.2604.5 — record from Abu's instance (not a fresh dev instance) so the dashboards reflect real installed-app state including the Cisco Security Cloud app (Splunkbase 7404 v3.6.6) running alongside Aegis, demonstrating the `cisco_ai_defense:aegis_verdict` sourcetype colocation that ADR-005 promises.
...
- `terminal-script.sh` hardcodes `AEGIS_AI_DEFENSE_MOCK=true` so the demo is reproducible without Cisco API access — judges who clone the repo and run the terminal script see the same BLOCK verdict the screencast shows, just from the mock backend. This is the "judges can re-run beat 2 + 3 themselves" guarantee in BDD #15.
- `AEGIS_PROFILE=fsi` (Financial Services Industry profile from story-mw-07) is the demo profile because: (a) FSI risk language maps directly to the Regulator Evidence Pack dashboard in beat 5 (SR 26-2 framing), (b) judges who don't know healthcare/HIPAA still recognize "customer SSN exfil" as a banking-clear violation, (c) the FSI profile's PII rule weighting produces a cleaner HIGH-severity verdict for the demo payload than HIPAA or PubSec would.
...
- Post-production: replace the README placeholder URL `https://youtube.com/watch?v=AEGIS_DEMO_PENDING` (from story-readme-01) with the real YouTube ID via `sed -i 's/AEGIS_DEMO_PENDING/<real-video-id>/g' README.md`. The `docs/demo/README.md` documents this sed swap in the post-production checklist section.
