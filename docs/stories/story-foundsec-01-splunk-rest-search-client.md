...
Given the test file
When  `uv run pytest packages/splunkgate_judges/tests/test_splunk_search_client.py -v` runs
Then  ≥ 10 tests pass and 0 fail

Given the new src files
When  `uv run mypy --strict packages/splunkgate_judges/src/splunkgate_judges/foundation_sec.py packages/splunkgate_judges/src/splunkgate_judges/_foundation_sec_errors.py` runs
Then  exit code is 0
...
When  `grep -rE "(mock|fake|dummy|hardcoded|simulated)" packages/splunkgate_judges/src/splunkgate_judges/foundation_sec.py` runs
Then  the output is empty

Given the src file
When  `grep -nE "verify\s*=\s*False" packages/splunkgate_judges/src/splunkgate_judges/foundation_sec.py` runs
Then  output is empty OR every match is preceded within 3 lines by the comment "SPLUNKGATE_DEV_INSECURE_TLS" (per architecture.md hard rule 7)
```
...
```bash
# Tests pass
uv run pytest packages/splunkgate_judges/tests/test_splunk_search_client.py -v 2>&1 | grep -cE "PASSED"
# Must output >= 10

# Constructor + error hierarchy sanity
uv run python -c "
from splunkgate_judges._foundation_sec_errors import (
...
for cls in (FoundationSecError, FoundationSecAuthError, FoundationSecSearchError, FoundationSecTimeoutError):
    assert issubclass(cls, SplunkGateError), cls
print('OK')
"
# Must print 'OK'

# Strict typecheck
uv run mypy --strict packages/splunkgate_judges/src/splunkgate_judges/foundation_sec.py packages/splunkgate_judges/src/splunkgate_judges/_foundation_sec_errors.py
# Must exit 0

# 400-LOC cap
for f in packages/splunkgate_judges/src/splunkgate_judges/foundation_sec.py packages/splunkgate_judges/src/splunkgate_judges/_foundation_sec_errors.py; do
  lines=$(wc -l < "$f")
...
grep -rE "(mock|fake|dummy|hardcoded|simulated)" packages/splunkgate_judges/src/splunkgate_judges/foundation_sec.py
# Must output nothing
```
...
- **Architecture.md hard rule 7** bans `verify=False` in production code paths. Honor the `SPLUNKGATE_DEV_INSECURE_TLS=1` env-var carve-out: read it inside `__init__`; if set AND `verify_tls=False` was passed explicitly, allow it but emit a single WARNING-level structlog event `foundsec.tls.insecure` referencing the env var.
