...
**Depends on:** story-cicd-01-build-pipeline-python-wheels, story-app-01-app-conf-and-metadata-skeleton
**Estimate:** ~1.5h
**Status:** PENDING
...
**As a** coding agent landing changes to `splunk_apps/splunkgate_app/`
...
- `.github/scripts/parse_appinspect.py` — NEW — Python script (~80-120 LOC, well under cap) that takes one argument `appinspect-report.json`, parses the JSON, counts entries with `result == "error"` (case-insensitive); exits 0 if zero errors, exits 1 if any errors. Emits `::error file=splunk_apps/splunkgate_app/...::<check_name>: <message>` GitHub annotation per error finding. Excludes findings already listed in `splunk_apps/splunkgate_app/.appinspect.expect.yaml` (verbatim string match on the `check_name` field). Uses only stdlib (`json`, `sys`, `pathlib`, `yaml` is acceptable since `pyyaml` is a uv-managed dep) — no `pydantic`, no network calls.
- `splunk_apps/splunkgate_app/README` — PRECONDITION (owned NEW by story-app-01) — the real Splunkbase-required README ships in app-01. This story assumes it exists.
- `splunk_apps/splunkgate_app/default/app.conf` — PRECONDITION (owned NEW by story-app-01) — the real `app.conf` with all five stanzas ships in app-01. This story assumes it exists. (Prior draft of cicd-05 used to create a minimal shell here; per audit synthesis Block C the duplicate is removed.)
- `splunk_apps/splunkgate_app/.appinspect.expect.yaml` — NEW — empty YAML file with header comment `# Manual-check exceptions list. Mirrors CIMplicity AI 2025 winner pattern. Per context/11-prior-art/01-build-a-thon-2025-deep-read.md.` The file MUST exist (parser script depends on it) but starts empty. EPIC-12 story `story-app-11-appinspect-expect-yaml-and-manual-checks.md` populates it.
...
Given the minimal `splunk_apps/splunkgate_app/` shell exists (app.conf + README + .appinspect.expect.yaml)
When  `uv run splunk-appinspect inspect splunk_apps/splunkgate_app/ --output-file appinspect-report.json --mode test --included-tags cloud --included-tags self-service --included-tags appapproval` runs
...
uv run splunk-appinspect inspect splunk_apps/splunkgate_app/ \
...
- The three `--included-tags` (`cloud`, `self-service`, `appapproval`) are spec-mandated (`docs/cicd-spec.md` line 129) because the SplunkGate app targets Splunk Cloud (cloud tag), self-install (self-service tag), and eventual Splunkbase listing (appapproval tag). Removing any tag relaxes the gate.
...
- **OWNERSHIP NOTE — audit synthesis Block C.** This story no longer creates the Splunk app shell (`app.conf` + `README`). Per the Block C consolidation, story-app-01-app-conf-and-metadata-skeleton.md is the sole NEW owner. cicd-05 depends_on app-01 and assumes the shell exists. Per `../../../context/05-splunk-core/08-app-packaging-and-conf-files.md`, `[launcher]` `version`, `description`, `author` are required; `[package]` `id` matches the app dir name — app-01's stanzas already satisfy all required fields so AppInspect will not error on missing required fields on the skeleton.
