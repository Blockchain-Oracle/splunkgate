...
**As a** coding agent writing code in any SplunkGate package
**I want to** have `.ruff.toml` and `mypy.ini` pre-tuned to the repo's standards (line-length 100, all ruff rules enabled except E501, mypy strict for `splunkgate_core`+`splunkgate_judges`, non-strict elsewhere)
...
- `mypy.ini` — NEW — global `python_version = 3.13`; non-strict baseline; per-module overrides flipping `strict = True` for `splunkgate_core.*` and `splunkgate_judges.*`
- `pyproject.toml` — UPDATE — add `[dependency-groups]` `dev = ["ruff", "mypy", ...]` (preserving existing dev group entries)
- `packages/splunkgate_core/src/splunkgate_core/__init__.py` — UPDATE — add `__all__: list[str] = []` so mypy strict has something typed to check against
- `packages/splunkgate_judges/src/splunkgate_judges/__init__.py` — UPDATE — add `__all__: list[str] = []` so mypy strict has something typed to check against
- `packages/splunkgate_core/tests/test_skeleton.py` — UPDATE — add a ruff-clean + mypy-clean assertion (typed test function with explicit return annotation)
...
When  `uv run mypy packages/splunkgate_core/src packages/splunkgate_judges/src` runs
Then  exit code is 0
And   stdout contains "Success: no issues found"

Given mypy.ini per-module override sets strict = True for splunkgate_core
When  `uv run python -c "import configparser; c=configparser.RawConfigParser(); c.read('mypy.ini'); print(c.get('mypy-splunkgate_core.*','strict'))"` runs
Then  stdout contains "True"

Given mypy.ini per-module override sets strict = True for splunkgate_judges
When  `uv run python -c "import configparser; c=configparser.RawConfigParser(); c.read('mypy.ini'); print(c.get('mypy-splunkgate_judges.*','strict'))"` runs
Then  stdout contains "True"
...
Given a deliberately broken Python file is created at /tmp/splunkgate_ruff_probe.py with content `import os\n\n` (unused import)
When  `uv run ruff check /tmp/splunkgate_ruff_probe.py` runs
...
# 3. mypy strict clean for splunkgate_core + splunkgate_judges
uv run mypy packages/splunkgate_core/src packages/splunkgate_judges/src
echo "mypy strict exit: $?"

# 4. mypy non-strict clean for the rest
uv run mypy packages/splunkgate_mw/src packages/splunkgate_mcp/src eval/src
echo "mypy non-strict exit: $?"

# 5. Strict-mode actually gates splunkgate_core: drop an Any-typed function and confirm mypy fails
cat > /tmp/splunkgate_mypy_probe.py <<'EOF'
from typing import Any
def bad(x: Any) -> Any:
    return x
EOF
mkdir -p packages/splunkgate_core/src/splunkgate_core/_probe
cp /tmp/splunkgate_mypy_probe.py packages/splunkgate_core/src/splunkgate_core/_probe/__init__.py
uv run mypy packages/splunkgate_core/src/splunkgate_core/_probe
# expect non-zero — strict-mode catches no-any-explicit / no-any-return
echo "mypy probe exit (expected non-zero): $?"
rm -rf packages/splunkgate_core/src/splunkgate_core/_probe

# 6. Ruff actually fires: probe with unused import
echo "import os" > /tmp/splunkgate_ruff_probe.py
uv run ruff check /tmp/splunkgate_ruff_probe.py
echo "ruff probe exit (expected non-zero): $?"
rm /tmp/splunkgate_ruff_probe.py
...
- Per `docs/architecture.md` § "Coding standards" hard rule 2, mypy must be `--strict` for `splunkgate_core` and `splunkgate_judges`. Per the same section, non-strict is acceptable for `splunkgate_mw`, `splunkgate_mcp`, and the Splunk app (constrained Python).
- Use mypy's `[mypy-splunkgate_core.*]` and `[mypy-splunkgate_judges.*]` per-module sections to flip `strict = True`, leaving the global `[mypy]` block non-strict. This is the simplest way to enforce the split without per-file overrides.
...
- DO NOT use `Any` in `splunkgate_core` or `splunkgate_judges` — this is a banned pattern per `docs/architecture.md` § "Banned patterns". The mypy-strict probe in the shell verification is there to enforce it.
