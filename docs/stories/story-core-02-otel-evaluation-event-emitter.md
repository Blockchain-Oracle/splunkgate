...
**I want to** call a single `emit_verdict_event(verdict, *, mcp_method_name=None, mcp_session_id=None)` from `aegis_core.otel` that emits a fully-OTel-GenAI-semantic-convention-compliant `gen_ai.evaluation.result` event
...
- `packages/aegis_core/src/aegis_core/otel.py` — NEW — `emit_verdict_event()` + `severity_to_score()` + `_build_attributes()` helpers. Wraps `opentelemetry.trace.get_current_span()` + `add_event()`. ≤ 280 LOC. mypy --strict clean.
- `packages/aegis_core/src/aegis_core/__init__.py` — UPDATE — re-export `emit_verdict_event`; update `__all__`
- `packages/aegis_core/tests/test_otel.py` — NEW — pytest cases using opentelemetry's `InMemorySpanExporter` to assert emitted event name/attrs match the OTel GenAI spec exactly. Includes both `mcp_method_name=None` (non-MCP origin) and the MCP-attrs-populated case.
- `packages/aegis_core/pyproject.toml` — UPDATE — add `opentelemetry-api`, `opentelemetry-sdk`, `opentelemetry-util-genai` to `dependencies`
...
When  `uv run python -c "from aegis_core.otel import emit_verdict_event; print('ok')"` runs
Then  exit code is 0
...
Then  `gen_ai.evaluation.name` is exactly "aegis.safety_verdict"
...
Given a Verdict is emitted with aegis-custom attrs (surface + rules + trace_id)
When  the event attributes are inspected
Then  `aegis.surface` equals the Verdict.surface value
And   `aegis.trace_id` is the string form of Verdict.trace_id
And   `aegis.rules` is a list-of-strings matching [r.rule for r in verdict.rules]

Given severity_to_score is called
When  `uv run python -c "from aegis_core.otel import severity_to_score; from aegis_core.verdict import Severity; print(severity_to_score(Severity.NONE_SEVERITY), severity_to_score(Severity.LOW), severity_to_score(Severity.MEDIUM), severity_to_score(Severity.HIGH))"` runs
Then  stdout contains four ascending floats (0.0 ≤ NONE < LOW < MED < HIGH ≤ 1.0)

Given the test suite runs
When  `uv run pytest packages/aegis_core/tests/test_otel.py -q` runs
Then  exit code is 0
...
When  `uv run ruff check packages/aegis_core/src/aegis_core/otel.py` runs
Then  exit code is 0
...
```bash
# 1. Import resolves
uv run python -c "from aegis_core.otel import emit_verdict_event, severity_to_score; print('ok')"

# 2. Severity → score is monotonic
uv run python -c "
from aegis_core.otel import severity_to_score
...
uv run pytest packages/aegis_core/tests/test_otel.py -q
...
- Per `docs/architecture.md` § "OTel emission shape", the aegis-custom attrs are `aegis.surface`, `aegis.rules` (list-of-strings of rule names), and `aegis.trace_id` (string-cast UUID). These are intentionally namespaced under `aegis.*` so they don't collide with any future upstream `gen_ai.*` additions.
...
- Per `docs/architecture.md` § "Banned patterns", no `Any` in `aegis_core`. Attribute dicts should be typed `dict[str, str | int | float | bool | list[str]]` (the OTel attribute value union per spec).
...
- Minimum 10 behavioral cases in `test_otel.py`:
  ...
  2. `gen_ai.evaluation.name` equals `aegis.safety_verdict`
  ...
  9. `aegis.surface` matches Verdict.surface
  10. `aegis.rules` is a list of rule-name strings
  11. `aegis.trace_id` is the string UUID
