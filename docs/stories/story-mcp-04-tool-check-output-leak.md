# Story — MCP tool `splunkgate_check_output_leak`
...
**I want to** call MCP tool `splunkgate_check_output_leak(output_text, sensitivity?)` that returns a typed `Verdict` — ALLOW pass-through / MODIFY with redacted output / BLOCK refusal
...
- `packages/splunkgate_mcp/src/splunkgate_mcp/tools/check_output_leak.py` — NEW — MCP tool definition; `class CheckOutputLeakInputs(BaseModel): output_text: str; sensitivity: Literal["default","fsi","hipaa","pubsec"] = "default"`; `async def check_output_leak(args: CheckOutputLeakInputs) -> Verdict`; calls `splunkgate_judges.ai_defense.inspect(text=output_text, rules_enabled=<profile-derived list>)` with the rule subset filtered to the documented PII/PHI/PCI rules ("PII", "PHI", "PCI") per the AI Defense 11-rule catalog; on MODIFY, populates `Verdict.modifications = {"redacted_output": "<text with PII/PHI/PCI replaced by [REDACTED:<rule>] tokens>"}`; `surface="mcp_check_output"`; emits OTel evaluation event
- `packages/splunkgate_mcp/src/splunkgate_mcp/server.py` — UPDATE — wire `check_output_leak.register(server)` into bootstrap
- `packages/splunkgate_mcp/tests/test_tool_check_output_leak.py` — NEW — ≥ 12 behavioral tests: tool discoverable, `outputSchema` equals `Verdict.model_json_schema()`, benign output → ALLOW, output containing US SSN → MODIFY with `modifications.redacted_output` containing `[REDACTED:PII]` and no original SSN substring, output containing PHI (DOB + diagnosis pattern) → MODIFY with `[REDACTED:PHI]`, output containing PCI (16-digit card) → MODIFY with `[REDACTED:PCI]`, sensitivity="hipaa" enables PHI rule explicitly, sensitivity="fsi" enables PCI rule explicitly, surface == "mcp_check_output", latency_ms populated, trace_id valid UUID, OTel evaluation event emitted with `splunkgate.surface == "mcp_check_output"`, in-band `isError: true` on AI Defense unreachable
...
Then  the returned list contains a tool named exactly "splunkgate_check_output_leak"
...
And   that event has attribute `splunkgate.surface` == "mcp_check_output"

Given the AI Defense client raises SplunkGateJudgeError (simulated upstream 503)
When  the tool is called
Then  the MCP result has isError == true (in-band per MCP spec)

Given the test file exists
When  `uv run pytest packages/splunkgate_mcp/tests/test_tool_check_output_leak.py -v` runs
Then  ≥ 12 tests pass and 0 fail

Given the production source
When  `wc -l packages/splunkgate_mcp/src/splunkgate_mcp/tools/check_output_leak.py` runs
Then  the line count is ≤ 400

Given the §14 grep runs on production code
When  `grep -rE "(mock|fake|dummy|hardcoded|simulated)" packages/splunkgate_mcp/src/splunkgate_mcp/tools/check_output_leak.py` runs
...
assert 'splunkgate_check_output_leak' in names, names
print('OK')
"
# Must print 'OK'

# Tests pass
uv run pytest packages/splunkgate_mcp/tests/test_tool_check_output_leak.py -v 2>&1 | grep -cE "PASSED"
# Must output >= 12

# Redaction round-trip smoke
uv run python -c "
import asyncio
from splunkgate_mcp.tools.check_output_leak import check_output_leak, CheckOutputLeakInputs
...
# 400-LOC cap
wc -l packages/splunkgate_mcp/src/splunkgate_mcp/tools/check_output_leak.py | awk '{ if ($1 > 400) exit 1 }'
# Must exit 0

# §14 clean
grep -rE "(mock|fake|dummy|hardcoded|simulated)" packages/splunkgate_mcp/src/splunkgate_mcp/tools/check_output_leak.py
# Must output nothing
```
...
- **Per `../../../context/06-splunk-ai-stack/03-splunk-mcp-server.md`, Splunk's official MCP Server is closed-source — we run our own server alongside, NOT register into it.** Tool name `splunkgate_check_output_leak` uses the `splunkgate_` prefix to coexist with `splunk_*` and `saia_*`.
...
- **Per `../../../context/10-standards/02-otel-genai-semantic-conventions.md`, MCP sub-convention attrs (`mcp.method.name`, `mcp.session.id`) co-emit with `gen_ai.evaluation.result` events.** Reuse `splunkgate_core.otel`.
...
- **Sensitivity profile → rules_enabled mapping:**
  ...
  - `"fsi"` → `["PII", "PCI"]`
  - `"hipaa"` → `["PII", "PHI"]`
  - `"pubsec"` → `["PII"]` (PubSec profile in `splunkgate_mw.profiles` is defined in EPIC-06; for MCP we only need PII at the base level here)
...
- Use `respx` for AI Defense HTTP mocking in tests. The `splunkgate_judges.ai_defense_mock` fixtures from EPIC-04 are reusable here.
