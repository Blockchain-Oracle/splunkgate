...
**As a** Aegis maintainer about to ship to a hackathon submission deadline
**I want to** prove the AI Defense client survives retries, circuit-breaker trips, and produces a correct `aegis_core.Verdict` against both an in-process FastAPI fake and (optionally, when an API key is set) the real Explorer Edition tenant
...
- `packages/aegis_judges/tests/integration/__init__.py` — NEW — empty
- `packages/aegis_judges/tests/integration/fake_ai_defense_server.py` — NEW — a FastAPI app exposing `POST /api/v1/inspect/chat`; reads a deterministic policy table; returns `InspectResponse`-shaped JSON; supports an injectable `failure_mode` (`"503-twice"`, `"401"`, `"timeout"`, `"happy"`) controlled via a header `X-Fake-AIDefense-Policy`
- `packages/aegis_judges/tests/integration/test_ai_defense_end_to_end.py` — NEW — ≥ 10 tests: client→fake happy path returns parsed `InspectResponse`; client→fake retries exhausted converts to circuit-breaker tick; 3 consecutive failures trip the breaker; recovery probe succeeds; `to_verdict()` mapping converts an `InspectResponse` to a valid `aegis_core.Verdict` with matching severity, rules, classifications, explanation; the verdict's `surface` field is set to `"judges_ai_defense"` (or per the surface enum); response includes `attack_technique` and `event_id` when present
- `packages/aegis_judges/src/aegis_judges/_verdict_mapping.py` — NEW — `inspect_response_to_verdict(resp: InspectResponse, *, trace_id, surface, latency_ms) -> Verdict` — the canonical mapping
- `packages/aegis_judges/tests/integration/test_ai_defense_live.py` — NEW — gated on `AEGIS_AI_DEFENSE_API_KEY`; skipped via `pytest.skip` when unset; when set, runs a single deterministic prompt against the configured region (env `AEGIS_AI_DEFENSE_REGION`, default `"us"`) and asserts `is_safe`/`severity` shape is one of the documented enums
...
Given AEGIS_AI_DEFENSE_API_KEY is unset
When  `uv run pytest packages/aegis_judges/tests/integration/test_ai_defense_live.py -v` runs
Then  every test is skipped with reason "AEGIS_AI_DEFENSE_API_KEY unset"
And   the exit code is 0

Given AEGIS_AI_DEFENSE_API_KEY is set to a real key
When  `uv run pytest packages/aegis_judges/tests/integration/test_ai_defense_live.py -v` runs
Then  the test runs against the real endpoint and asserts shape compliance

Given the integration test file
When  `uv run pytest packages/aegis_judges/tests/integration/test_ai_defense_end_to_end.py -v` runs
Then  ≥ 10 tests pass and 0 fail

Given the new src files
When  `uv run mypy --strict packages/aegis_judges/src/aegis_judges/_verdict_mapping.py` runs
Then  exit code is 0
...
uv run pytest packages/aegis_judges/tests/integration/test_ai_defense_end_to_end.py -v 2>&1 | grep -cE "PASSED"
# Must output >= 10

# Live suite gates correctly (no key → skipped, exit 0)
env -u AEGIS_AI_DEFENSE_API_KEY uv run pytest packages/aegis_judges/tests/integration/test_ai_defense_live.py -v 2>&1 | grep -E "(SKIPPED|skipped)"
# Must print at least one SKIPPED line; exit code must be 0
env -u AEGIS_AI_DEFENSE_API_KEY uv run pytest packages/aegis_judges/tests/integration/test_ai_defense_live.py
# Must exit 0
...
from aegis_judges._verdict_mapping import inspect_response_to_verdict
...
# Strict typecheck
uv run mypy --strict packages/aegis_judges/src/aegis_judges/_verdict_mapping.py
# Must exit 0

# 400-LOC cap
for f in packages/aegis_judges/src/aegis_judges/_verdict_mapping.py packages/aegis_judges/tests/integration/fake_ai_defense_server.py; do
  lines=$(wc -l < "$f")
...
- The `inspect_response_to_verdict` mapping is the boundary between the AI Defense schema and the Aegis `Verdict` schema. Severity → severity is a 1:1 mapping (both enums share string values). `is_safe=True` → `VerdictLabel.ALLOW`. `is_safe=False && severity in {LOW,MEDIUM}` → `VerdictLabel.REVIEW`. `is_safe=False && severity==HIGH` → `VerdictLabel.BLOCK`. The mapping rule is documented in `architecture.md` § "API schemas" and `docs/PRD.md` — re-cite there if the rule shifts.
...
- Live test gating: use `pytest.mark.skipif(not os.environ.get("AEGIS_AI_DEFENSE_API_KEY"), reason="AEGIS_AI_DEFENSE_API_KEY unset")` on each test function, not on the module — that way `pytest -v` still lists the gated tests.
- Cisco AI Defense Explorer Edition (`https://explorer.aidefense.cisco.com/`, March 23 2026 launch, free US-corp-email signup) is the demo-recording path — generate the live API key from Explorer Edition's UI and pass via `AEGIS_AI_DEFENSE_API_KEY` to run the live test segment of the screencast. The Explorer key is rate-limited but should comfortably accommodate one demo run.
