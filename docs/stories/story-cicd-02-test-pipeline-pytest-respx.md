...
**I want to** push a commit and see a green `test` matrix job in GitHub Actions with coverage reported to Codecov for `aegis_core` + `aegis_judges`
...
- `packages/aegis_core/tests/__init__.py` — NEW — empty file (Python package marker)
- `packages/aegis_core/tests/test_smoke.py` — NEW — single test `def test_version_present(): import aegis_core; assert aegis_core.__version__ == "0.0.0"` so the matrix run for `aegis_core` actually executes one test case (proves wiring)
- `packages/aegis_judges/tests/__init__.py` — NEW — empty
- `packages/aegis_judges/tests/test_smoke.py` — NEW — single smoke test asserting the package import succeeds
- `packages/aegis_mw/tests/__init__.py` — NEW — empty
- `packages/aegis_mw/tests/test_smoke.py` — NEW — single smoke test
- `packages/aegis_mcp/tests/__init__.py` — NEW — empty
- `packages/aegis_mcp/tests/test_smoke.py` — NEW — single smoke test
- `codecov.yml` — NEW — set `coverage.status.project.default.target: 70%` and `flags: [aegis_core, aegis_judges]` per `docs/cicd-spec.md` § "Required green checks before merge"
...
When  `uv run pytest packages/aegis_core --cov=aegis_core --cov-report=xml --cov-fail-under=70` runs
Then  exit code is 0
And   `coverage.xml` exists in the working directory

Given `uv run pytest packages/aegis_judges` runs
When  the run completes
Then  exit code is 0
And   stdout contains `1 passed` (one smoke test)

Given `uv run pytest packages/aegis_mw packages/aegis_mcp` runs
When  the run completes
Then  exit code is 0
...
And   the codecov upload step only fires for `aegis_core` and `aegis_judges` matrix entries (verified via `if: ${{ matrix.package == 'aegis_core' || matrix.package == 'aegis_judges' }}`)
...
uv run pytest packages/aegis_core --cov=aegis_core --cov-report=xml --cov-fail-under=70
test -s coverage.xml
uv run pytest packages/aegis_judges --cov=aegis_judges --cov-report=xml --cov-fail-under=70
test -s coverage.xml

# 4. CI YAML wires the matrix correctly
grep -q 'matrix:' .github/workflows/ci.yml
grep -q 'package: \[aegis_core, aegis_judges, aegis_mw, aegis_mcp\]' .github/workflows/ci.yml
grep -q 'codecov/codecov-action' .github/workflows/ci.yml
grep -q "matrix.package == 'aegis_core' || matrix.package == 'aegis_judges'" .github/workflows/ci.yml
...
- `asyncio_mode = "auto"` matters because `aegis_judges` is fully async (httpx async client) — every test that touches the client must be an `async def`. The alternative (`strict` mode) requires `@pytest.mark.asyncio` on every test; `auto` removes the boilerplate.
- Codecov upload is gated by `if: matrix.package == 'aegis_core' || matrix.package == 'aegis_judges'` because those two packages have `--strict` mypy and are load-bearing; `aegis_mw` and `aegis_mcp` have non-strict typing per `docs/architecture.md` § "Hard rules" #2 and are slower-moving for coverage tracking.
