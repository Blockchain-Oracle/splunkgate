...
- `scripts/bump_version.sh` — NEW — bash script (~80 LOC, under cap) that takes a SemVer arg (e.g., `0.1.0`) and updates: (1) `packages/splunkgate_core/pyproject.toml` `version = "..."`; (2) same for the other three packages; (3) `splunk_apps/splunkgate_app/default/app.conf` `[launcher] version = ...`; (4) creates a `git commit -m "chore(release): bump to vX.Y.Z"`. Uses `sed -i` (Linux) or `sed -i ""` (macOS) — detect via `uname`. Refuses to run if working tree is dirty (`git status --porcelain` non-empty).
...
- The `tests/test_bump_version.sh` test copies fixtures to `/tmp/splunkgate_bump_test_$$/` (PID suffix for isolation), invokes the script with `--cwd` or by `cd`-ing inside a subshell, then `grep -q 'version = "9.9.9"'` across the 5 fixture files. Use `mktemp -d` for atomic temp dir creation.
...
- Per `../../../context/11-prior-art/01-build-a-thon-2025-deep-read.md`, no 2025 winner shipped signed artifacts via sigstore — this differentiates SplunkGate's submission as production-shape. Mention in the README (EPIC-11) under a "Provenance" section.
