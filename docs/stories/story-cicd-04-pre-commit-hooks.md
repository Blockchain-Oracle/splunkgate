...
- `.pre-commit-config.yaml` — NEW — **sole owner per audit synthesis Block C**. Verbatim adapted from `docs/cicd-spec.md` § "Pre-commit hooks" lines 406-442. Five hook groups: (1) astral-sh/ruff-pre-commit `v0.7.0` for `ruff --fix` + `ruff-format`; (2) pre-commit/mirrors-mypy `v1.13.0` strict-mode on `packages/(splunkgate_core|splunkgate_judges)/src/`; (3) local hook with **canonical id `check-loc-400`** calling **`.github/scripts/check_loc.py`** (the Python script shipped by story-cicd-03; NOT the `.sh` form — there is no `.sh` form, cicd-03 ships only `.py`); (4) local hook `no-print` rejecting `print(` in `packages/splunkgate_*/src/` except `*_mock.py` / test files / lines mentioning `structlog`; (5) gitleaks/gitleaks `v8.21.2`. Story-skel-04 UPDATES this same file later to add additional language-specific hooks but does NOT re-define `check-loc-400`.
...
Given the no-print fixture `tests/fixtures/pre_commit_violators/has_print.py.fixture` is copied to `packages/splunkgate_judges/src/splunkgate_judges/_violator.py`
...
Given the 400-LOC oversized fixture is copied into `packages/splunkgate_core/src/splunkgate_core/_oversized.py` (re-use the fixture from cicd-03)
...
cp tests/fixtures/pre_commit_violators/has_print.py.fixture packages/splunkgate_judges/src/splunkgate_judges/_violator.py
if uv run pre-commit run no-print --all-files; then
  echo "FAIL: no-print hook didn't fire"; rm packages/splunkgate_judges/src/splunkgate_judges/_violator.py; exit 1
fi
rm packages/splunkgate_judges/src/splunkgate_judges/_violator.py

# 4. check-loc-400 hook fires on an oversized file
cp tests/fixtures/loc_oversized.py.fixture packages/splunkgate_core/src/splunkgate_core/_oversized.py
if uv run pre-commit run check-loc-400 --all-files; then
  echo "FAIL: check-loc-400 hook didn't fire"; rm packages/splunkgate_core/src/splunkgate_core/_oversized.py; exit 1
fi
rm packages/splunkgate_core/src/splunkgate_core/_oversized.py

# 5. gitleaks fires on a realistic-shaped secret
cp tests/fixtures/pre_commit_violators/has_secret.py.fixture packages/splunkgate_core/src/splunkgate_core/_secret.py
git add packages/splunkgate_core/src/splunkgate_core/_secret.py
if uv run pre-commit run gitleaks; then
  echo "FAIL: gitleaks didn't catch the secret-shaped string"
  git restore --staged packages/splunkgate_core/src/splunkgate_core/_secret.py
  rm packages/splunkgate_core/src/splunkgate_core/_secret.py
  exit 1
fi
git restore --staged packages/splunkgate_core/src/splunkgate_core/_secret.py
rm packages/splunkgate_core/src/splunkgate_core/_secret.py
...
- `mypy` hook `additional_dependencies: [pydantic, httpx, structlog]` is required because pre-commit runs mypy in an isolated env without the project's deps. The list mirrors the load-bearing runtime libs of `splunkgate_core` + `splunkgate_judges` per `docs/architecture.md` § "Required external libraries".
