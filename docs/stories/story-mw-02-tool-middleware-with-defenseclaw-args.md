...
- `packages/aegis_mw/src/aegis_mw/tool_middleware.py` — NEW — implements `SafetyToolMiddleware(AgentMiddleware)` with concrete `async def tool_middleware(self, request: ToolRequest, handler: ToolMiddlewareHandler) -> ToolResponse`; inspects `request.call.name` + `request.call.args`; runs DefenseClaw rule-pack (via `aegis_judges.defenseclaw_backend.evaluate_tool_call(name, args)`); if rule-pack hit is HIGH or above OR `config.escalate_on_first_pass_hit` is set, escalates to `aegis_judges.ai_defense.inspect(text=serialize(call))`; assembles a `Verdict` (`surface="mw_tool"`, `severity`, `rules`, `latency_ms`) and emits via `aegis_core.otel.emit_evaluation_result(verdict)`; behavior by verdict label: `BLOCK` → raises `aegis_core.errors.ToolBlockedByAegis(verdict)` (a subclass of `AegisError`); `MODIFY` → returns a `ToolMiddlewareHandler` call with sanitized args (`verdict.modifications["sanitized_args"]`); `ALLOW` → passes through to `await handler(request)`
- `packages/aegis_mw/src/aegis_mw/_serialization.py` — NEW — small helper `serialize_tool_call(call: ToolCall) -> str` that returns `f"{call.name}({json.dumps(call.args, sort_keys=True)})"`, used by both tool and model middleware
- `packages/aegis_mw/src/aegis_mw/__init__.py` — UPDATE — make sure `SafetyToolMiddleware` re-export still resolves to the new implementation in `tool_middleware.py` (replace the stub from story-mw-01)
- `packages/aegis_core/src/aegis_core/errors.py` — UPDATE — add `ToolBlockedByAegis(AegisError)` taking `verdict: Verdict` constructor arg
- `packages/aegis_mw/tests/test_tool_middleware.py` — NEW — ≥ 14 behavioral tests using `respx` to mock the AI Defense HTTP endpoint and a fake `ToolMiddlewareHandler`: ALLOW path passes through unchanged; BLOCK path raises `ToolBlockedByAegis`; MODIFY path returns a `ToolResponse` whose underlying call used the sanitized args; OTel emission asserted via in-memory exporter; trace_id from `agent_middleware` (mocked) propagates onto the verdict; DefenseClaw-only hit (no AI Defense escalation) emits a verdict with `source="defenseclaw_regex"`; AI Defense-escalated hit emits a verdict with `source="ai_defense"`; profile="default" routes correctly; latency_ms is populated > 0
- `packages/aegis_mw/tests/conftest.py` — NEW or UPDATE — fixtures for in-memory OTel exporter, fake `ToolRequest`/`ToolMiddlewareHandler`, respx-mocked AI Defense client
...
When  `uv run pytest packages/aegis_mw/tests/test_tool_middleware.py::test_allow_passthrough -v` runs
...
When  `uv run pytest packages/aegis_mw/tests/test_tool_middleware.py::test_block_raises -v` runs
Then  the test passes; aegis_core.errors.ToolBlockedByAegis is raised; the inner handler is NOT called

Given SafetyToolMiddleware wraps a ToolCall whose verdict is MODIFY with sanitized args
When  `uv run pytest packages/aegis_mw/tests/test_tool_middleware.py::test_modify_returns_sanitized -v` runs
...
Then  exactly one `gen_ai.evaluation.result` event is emitted per invocation with `gen_ai.evaluation.name="aegis.safety_verdict"`, `aegis.surface="mw_tool"`, and a populated `aegis.trace_id`
...
When  `uv run pytest packages/aegis_mw/tests/test_tool_middleware.py -v` runs
...
When  `grep -rE "(mock|fake|dummy|hardcoded|simulated)" packages/aegis_mw/src/aegis_mw/tool_middleware.py packages/aegis_mw/src/aegis_mw/_serialization.py` runs
...
uv run python -c "
import inspect
from aegis_mw import SafetyToolMiddleware
...
# Tests pass
uv run pytest packages/aegis_mw/tests/test_tool_middleware.py -v 2>&1 | grep -cE "PASSED"
# Must output >= 14

# Tool middleware module is ≤ 400 LOC
wc -l packages/aegis_mw/src/aegis_mw/tool_middleware.py | awk '{ if ($1 > 400) exit 1 }'
# Must exit 0

# §14 clean
grep -rE "(mock|fake|dummy|hardcoded|simulated)" packages/aegis_mw/src/aegis_mw/tool_middleware.py packages/aegis_mw/src/aegis_mw/_serialization.py
# Must output nothing

# Lint + typecheck
uv run ruff check packages/aegis_mw/
uv run mypy packages/aegis_mw/src/aegis_mw/tool_middleware.py
# Both must exit 0
```
...
- **Per `../../../context/02-agent-frameworks/06-splunklib-ai-deep-read.md` §"Convenience hooks", a hook can stop the agent loop by raising any exception inside the user function.** Raising `ToolBlockedByAegis(verdict)` is the BLOCK semantic. The agent loop unwinds with our typed exception — downstream callers can inspect `e.verdict`.
...
- DefenseClaw is a Go binary — we call into its rule engine via the wrapper `aegis_judges.defenseclaw_backend.evaluate_tool_call(name: str, args: dict) -> RuleHit | None` shipped by EPIC-08. For this story, treat that import as the contract; respx fixture can stand in if EPIC-08 hasn't landed yet (mark with TODO + issue link).
- The AI Defense client (`aegis_judges.ai_defense.inspect`) defaults to mock=True per ADR-006; tests run without credentials. The respx fixtures from `story-judges-04-ai-defense-mock-respx-fixtures.md` are the canonical mock shape.
...
- For the MODIFY path: `verdict.modifications` is a `dict | None`. When MODIFY, the contract is `verdict.modifications["sanitized_args"]: dict` — the agent re-runs the tool with these args. If `sanitized_args` is missing, raise `AegisError` rather than silently passing through original args.
...
- structlog: bind `trace_id` from `request.state` (LangChain v1 surfaces it via `state.messages[-1].extras` or via the `aegis_mw.agent_middleware` from story-mw-06) and log the verdict label at INFO.
