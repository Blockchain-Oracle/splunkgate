...
**As a** coding agent writing any Aegis surface
**I want to** call `from aegis_core.logging import get_logger` and get a pre-configured `structlog` BoundLogger that (a) renders JSON in prod and console-pretty in dev (env-toggled), (b) emits stable key names (`event`, `verdict`, `severity`, `trace_id`, ...), and (c) auto-injects the current `trace_id` (from `aegis_core.trace`) into every log record
**So that** the architecture's hard rule "All log lines use `structlog` with stable key names" is enforced by API shape (not by reviewer vigilance), and SOC analysts searching `cisco_ai_defense:aegis_verdict` events can join across log + Verdict + OTel event via a single key
...
- `packages/aegis_core/src/aegis_core/logging.py` — NEW — `configure_logging(*, dev_mode: bool | None = None)`, `get_logger(name: str)`, a `_trace_id_processor` that pulls `current_trace_id()` from `aegis_core.trace` into every record. ≤ 200 LOC. mypy --strict clean.
- `packages/aegis_core/src/aegis_core/__init__.py` — UPDATE — re-export `get_logger`, `configure_logging`; update `__all__`
- `packages/aegis_core/tests/test_logging.py` — NEW — pytest cases using `structlog.testing.capture_logs()` (or `LogCapture` processor) to assert key shape, JSON-vs-console toggling, auto-injected trace_id presence
- `packages/aegis_core/pyproject.toml` — UPDATE — add `structlog` to `dependencies`
...
When  `uv run python -c "from aegis_core.logging import get_logger, configure_logging; print('ok')"` runs
Then  exit code is 0
...
When  `uv run pytest packages/aegis_core/tests/test_logging.py -q` runs
Then  exit code is 0
...
When  `uv run ruff check packages/aegis_core/src/aegis_core/logging.py` runs
Then  exit code is 0
...
When  `grep -nE '\bprint\(' packages/aegis_core/src/aegis_core/logging.py` runs
...
```bash
# 1. Imports resolve
uv run python -c "from aegis_core.logging import get_logger, configure_logging; print('ok')"
...
AEGIS_LOG_FORMAT=console uv run python -c "
...
from structlog.testing import capture_logs
from aegis_core.logging import configure_logging, get_logger
from aegis_core.trace import new_trace_id, trace_context
configure_logging(dev_mode=False)
log = get_logger('test')
tid = new_trace_id()
...
# 5. Test suite passes
uv run pytest packages/aegis_core/tests/test_logging.py -q
...
# 10. §14 clean
grep -E "(mock|fake|dummy|hardcoded|simulated)" packages/aegis_core/src/aegis_core/logging.py
# Must output nothing
```
...
- Per `docs/architecture.md` § "Banned patterns", no `Any` in `aegis_core`. structlog's BoundLogger has `Any`-ish call signatures by design — use `cast(structlog.BoundLogger, ...)` where mypy --strict complains; document the cast inline (justification comment required by CLAUDE.md).
- Minimum 8 behavioral cases in `test_logging.py`:
  ...
  4. `AEGIS_LOG_FORMAT=json` env var overrides dev_mode auto-detect to JSON
  5. `AEGIS_LOG_FORMAT=console` env var overrides dev_mode auto-detect to console
