...
**As a** developer evaluating SplunkGate from outside the Splunk ecosystem (Claude Desktop user, Cursor IDE user, any MCP-aware host)
**I want to** drop a verbatim JSON config snippet into my MCP client's settings file and have SplunkGate's four tools (`splunkgate_score_prompt_injection`, `splunkgate_judge_tool_call`, `splunkgate_check_output_leak`, `splunkgate_audit_trace`) appear and work — with one stdio example and one HTTP example per client, plus a troubleshooting section that covers the actual failure modes
**So that** the MCP-bonus-prize pitch ("any MCP client can call us") is self-service — a judge can `cmd+,` into Claude Desktop, paste the config, and see SplunkGate verdicts inside Claude Desktop in under 60 seconds
...
- `docs/integrations/README.md` — NEW — markdown only; one-paragraph index pointing at `claude-desktop.md` and `cursor.md`; one-paragraph note citing `context/06-splunk-ai-stack/03-splunk-mcp-server.md` that Splunk's MCP Server is closed-source and SplunkGate runs alongside, not registered into; tool-name table mapping each of the 4 SplunkGate tools to the matching Surface 1 middleware function (so cross-surface users see the equivalence)
- `docs/integrations/_examples/splunkgate-mcp-stdio.json` — NEW — pure JSON file with the stdio config (importable by README + linkable from the integration docs)
- `docs/integrations/_examples/splunkgate-mcp-http.json` — NEW — pure JSON file with the HTTP config
- `docs/integrations/_examples/splunkgate-mcp-coexist-with-splunk.json` — NEW — pure JSON file showing the coexistence pattern: both `splunk-mcp-server` and `splunkgate-mcp-server` keys in the same `mcpServers` block
- `docs/integrations/README.md` — UPDATE — additionally include a section titled exactly **"Coexistence with Splunk MCP Server"** that enumerates verbatim the 10 native Splunk MCP Server tools and 4 SAIA tools that SplunkGate tools coexist alongside (so a judge or developer sees no name collisions and the two surfaces are clearly partitioned). Verbatim tool lists (cite `../../../context/06-splunk-ai-stack/03-splunk-mcp-server.md`):
  ...
  - **SplunkGate (4 `splunkgate_*` tools, this story's surface):** `splunkgate_score_prompt_injection`, `splunkgate_judge_tool_call`, `splunkgate_check_output_leak`, `splunkgate_audit_trace`
  No name collisions between any of the three sets — the three prefixes (`splunk_`, `saia_`, `splunkgate_`) partition cleanly.
...
Then  the output contains the substring "splunkgate_score_prompt_injection"
And   the substring "splunkgate_judge_tool_call"
And   the substring "splunkgate_check_output_leak"
And   the substring "splunkgate_audit_trace"

Given docs/integrations/claude-desktop.md exists
When  `grep -E '"command".*"python".*"-m".*"splunkgate_mcp"' docs/integrations/claude-desktop.md` runs
...
Then  the output contains "splunkgate_score_prompt_injection"
...
When  `python -c "import json; json.load(open('docs/integrations/_examples/splunkgate-mcp-stdio.json'))"` runs
Then  exit code is 0 (valid JSON)

Given the example JSON files exist
When  `python -c "import json; json.load(open('docs/integrations/_examples/splunkgate-mcp-http.json'))"` runs
Then  exit code is 0

Given the example JSON files exist
When  `python -c "import json; json.load(open('docs/integrations/_examples/splunkgate-mcp-coexist-with-splunk.json'))"` runs
Then  exit code is 0

Given the coexistence example
When  `python -c "import json; cfg=json.load(open('docs/integrations/_examples/splunkgate-mcp-coexist-with-splunk.json')); print(sorted(cfg['mcpServers'].keys()))"` runs
Then  the output contains both "splunkgate-mcp-server" and "splunk-mcp-server"
...
for tool in splunkgate_score_prompt_injection splunkgate_judge_tool_call splunkgate_check_output_leak splunkgate_audit_trace; do
...
for f in docs/integrations/_examples/splunkgate-mcp-stdio.json \
         docs/integrations/_examples/splunkgate-mcp-http.json \
         docs/integrations/_examples/splunkgate-mcp-coexist-with-splunk.json; do
...
python -c "
import json
cfg = json.load(open('docs/integrations/_examples/splunkgate-mcp-coexist-with-splunk.json'))
servers = cfg['mcpServers']
assert 'splunkgate-mcp-server' in servers
...
- **Stdio example must point at the actual entrypoint from `story-mcp-01`:** `python -m splunkgate_mcp` with `env: {"SPLUNKGATE_MCP_TRANSPORT": "stdio"}`. If the entrypoint changed in `mcp-01`'s implementation, update both example JSON files to match — do NOT diverge from the actual code.
- **HTTP example default port is `8765`** (SplunkGate's chosen port; not conflicting with Splunk's `:8089/services/mcp`). Document this in the troubleshooting section and as a JSON comment in the example file (JSON-with-comments is unsupported, so put the port-collision note in the surrounding markdown, not inside the JSON file).
- **`docs/integrations/README.md` includes a tool-equivalence table** mapping each MCP tool to the matching Surface 1 middleware function — so a developer who reads the index sees "MCP `splunkgate_score_prompt_injection` == middleware `splunkgate_mw.model_middleware.pre_inference_scan`". This is the cross-surface conceptual map.
