# Story — collections.conf KV-store schema for aegis_verdict_history
...
**I want to** see every Aegis verdict (with retention metadata + jurisdictional_tag for HIPAA/PCI/FSI profiles) persisted in a queryable Splunk KV-store collection that survives index rotation
...
- `splunk_apps/aegis_app/default/collections.conf` — NEW — one stanza `[aegis_verdict_history]` defining the KV-store collection schema, one row per verdict. Fields: `_key` (string, format `trace_id#timestamp_epoch`), `_user` (string, owner), `last_update` (time), `trace_id` (string, UUID), `timestamp` (time), `agent_id` (string), `surface` (string, one of mw_model/mw_tool/mw_subagent/mcp_score/mcp_judge_tool/mcp_check_output/mcp_audit/defenseclaw), `verdict_label` (string, ALLOW/BLOCK/MODIFY/REVIEW), `severity` (string, NONE_SEVERITY/LOW/MEDIUM/HIGH), `severity_score` (number, 0-3), `rules` (string, comma-separated for KV-store flat shape), `classifications` (string, comma-separated), `explanation` (string, max 4000 chars), `latency_ms` (number), `jurisdictional_tag` (string, FSI/HIPAA/PUBSEC/PCI/NONE for profile-gated dashboards in story app-07), `retention_until` (time, computed at insert: `now() + retention_years * 31536000s`, retention_years from profile lookup). Plus a `[aegis_profile_index]` collection (small lookup-style KV-store) mapping `jurisdictional_tag` → `retention_years`, `applicable_regs`, used by the Regulator Evidence Pack dashboard.
- `splunk_apps/aegis_app/default/transforms.conf` — UPDATE — append two lookup definitions: `[aegis_verdict_history_lookup]` (`external_type = kvstore`, `collection = aegis_verdict_history`, `fields_list = _key, _user, last_update, trace_id, timestamp, agent_id, surface, verdict_label, severity, severity_score, rules, classifications, explanation, latency_ms, jurisdictional_tag, retention_until`) and `[aegis_profile_index_lookup]` (`external_type = kvstore`, `collection = aegis_profile_index`, `fields_list = _key, jurisdictional_tag, retention_years, applicable_regs`). Mirrors DNS Guard's `transforms.conf` lookup-stanza pattern (`../inspiration/Splunk-DNS-Guard-AI/Splunk-DNS-Guard-AI/default/transforms.conf`).
- `splunk_apps/aegis_app/lookups/aegis_profile_seed.csv` — NEW — 4-row seed CSV bootstrapping the `aegis_profile_index` KV-store on first install (jurisdictional_tag, retention_years, applicable_regs): `FSI,7,"FFIEC-AIML;SR-26-2"`, `HIPAA,7,"HIPAA-SafeHarbor-18"`, `PUBSEC,3,"NIST-AI-RMF"`, `PCI,1,"PCI-DSS-11.x"`. Wired via a one-shot saved search (`[Aegis - Bootstrap profile index]` in savedsearches.conf — orchestrator agent may need to update story app-03 if not already there; if so, flag in PR for follow-up).
...
```
Given splunk_apps/aegis_app/default/collections.conf exists
When  grep -cE "^\[(aegis_verdict_history|aegis_profile_index)\]$" runs
Then  count == 2

Given the [aegis_verdict_history] stanza
...
Given splunk_apps/aegis_app/default/transforms.conf has the lookup stanzas appended
When  grep -cE "^\[aegis_(verdict_history|profile_index)_lookup\]$" runs
Then  count == 2

Given the [aegis_verdict_history_lookup] stanza
When  grep "external_type = kvstore\|collection = aegis_verdict_history" runs
Then  both lines present

Given splunk_apps/aegis_app/lookups/aegis_profile_seed.csv exists
When  wc -l runs against it
Then  output is 5 (header + 4 data rows)

Given aegis_profile_seed.csv
When  grep -cE "^(FSI|HIPAA|PUBSEC|PCI)," runs
Then  count == 4 (all four profile rows present)

Given a Splunk container with aegis_app installed
When  the SPL "| inputlookup aegis_verdict_history_lookup | head 1" runs
...
# 1. Files exist
test -f splunk_apps/aegis_app/default/collections.conf
test -f splunk_apps/aegis_app/lookups/aegis_profile_seed.csv

# 2. Collections.conf has both stanzas
grep -q '^\[aegis_verdict_history\]$' splunk_apps/aegis_app/default/collections.conf
grep -q '^\[aegis_profile_index\]$' splunk_apps/aegis_app/default/collections.conf

# 3. aegis_verdict_history declares all required fields
for field in '_key = string' '_user = string' 'last_update = time' 'trace_id = string' \
  ...
  grep -q "^field\.${field}$" splunk_apps/aegis_app/default/collections.conf || \
    { echo "Missing field: $field"; exit 1; }
done

# 4. aegis_profile_index has its fields
...
grep -q '^\[aegis_verdict_history_lookup\]$' splunk_apps/aegis_app/default/transforms.conf
grep -q '^\[aegis_profile_index_lookup\]$' splunk_apps/aegis_app/default/transforms.conf
grep -A4 '^\[aegis_verdict_history_lookup\]$' splunk_apps/aegis_app/default/transforms.conf | grep -q 'external_type = kvstore'
grep -A4 '^\[aegis_verdict_history_lookup\]$' splunk_apps/aegis_app/default/transforms.conf | grep -q 'collection = aegis_verdict_history'

# 6. Seed CSV has correct row count and profile names
test "$(wc -l < splunk_apps/aegis_app/lookups/aegis_profile_seed.csv)" -eq 5
grep -q '^FSI,7,' splunk_apps/aegis_app/lookups/aegis_profile_seed.csv
grep -q '^HIPAA,7,' splunk_apps/aegis_app/lookups/aegis_profile_seed.csv
grep -q '^PUBSEC,3,' splunk_apps/aegis_app/lookups/aegis_profile_seed.csv
grep -q '^PCI,1,' splunk_apps/aegis_app/lookups/aegis_profile_seed.csv
head -1 splunk_apps/aegis_app/lookups/aegis_profile_seed.csv | grep -q '^jurisdictional_tag,retention_years,applicable_regs$'

# 7. Live KV-store readback (gated on AEGIS_SPLUNK_API_TOKEN)
...
collection = svc.kvstore["aegis_verdict_history"]
...
# 9. 400-LOC sanity
test "$(wc -l < splunk_apps/aegis_app/default/collections.conf)" -le 400
```
...
- Per `docs/architecture.md` § "Repo structure > splunk_apps/aegis_app/", `collections.conf` defines KV-store schemas; `lookups/*.csv` provides bootstrap seed rows; lookups defined via `transforms.conf` `[<name>_lookup]` stanzas with `external_type = kvstore`. Mirrors DNS Guard's exact wiring.
...
- Per `../../../context/05-splunk-core/08-app-packaging-and-conf-files.md`, the lookups directory must be at `splunk_apps/aegis_app/lookups/` (not under `default/`). Splunkbase rejects packages with lookups misplaced.
...
- DNS Guard uses `_key = domain#anomalous_type` as the dedupe key. Aegis uses `_key = trace_id#timestamp_epoch` — same composite-key pattern, ensures one row per verdict event even if the saved search runs twice.
- The `[aegis_profile_index]` collection is intentionally small (4 rows) — it's a lookup table, not a metrics store. KV-store handles small lookup tables faster than CSV lookups because it doesn't lock the file during reads.
...
- If the `[Aegis - Bootstrap profile index]` saved search isn't already in story app-03's savedsearches.conf, do NOT add it here — flag in the PR description for story app-03 to add. This story owns the schema, not the bootstrapping; cross-story changes need explicit re-scoping per the file modification map rule.
