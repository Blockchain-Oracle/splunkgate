...
- **ADR location:** ADR template (`docs/adrs/_template.md`) and `docs/adrs/README.md` live under `docs/adrs/`. The `docs/adrs/` directory itself is created by `story-ops-02-github-secrets-and-adr-template.md` (Block D of the audit-synthesis plan), which OWNS those artifacts. This story does NOT create `docs/adrs/` — it only references the convention in CLAUDE.md (e.g., "New architecture decisions go in `docs/adrs/NNNN-<slug>.md`, copy from `docs/adrs/_template.md`"). Do NOT duplicate the template here.
