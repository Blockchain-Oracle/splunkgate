...
APP_DIR = REPO_ROOT / "splunk_apps" / "splunkgate_app"
...
def test_postprocessor_succeeds_on_clean_fixture() -> None:
    ...
    rc = subprocess.run(
        [
            ...
            "--summary-file",
            str(Path("/tmp") / "splunkgate-appinspect-clean-summary.txt"),  # noqa: S108
        ],
        check=False,
    ).returncode
    assert rc == 0
...
def test_postprocessor_fails_on_error_fixture() -> None:
    ...
    rc = subprocess.run(
        [
            ...
            str(APP_DIR / ".appinspect.expect.yaml"),
            "--summary-file",
            str(Path("/tmp") / "splunkgate-appinspect-error-summary.txt"),  # noqa: S108
        ],
        check=False,
    ).returncode
    assert rc == 1
...
def test_postprocessor_respects_expect_yaml_suppression(tmp_path: Path) -> None:
    ...
    rc = subprocess.run(
        [
            ...
            "--summary-file",
            str(Path("/tmp") / "splunkgate-appinspect-suppressed-summary.txt"),  # noqa: S108
        ],
        check=False,
    ).returncode
    assert rc == 0
